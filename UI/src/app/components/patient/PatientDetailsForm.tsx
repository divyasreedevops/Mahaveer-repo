...
export function PatientDetailsForm() {
  ...
  if (
    ...
    currentPatient?.aadhaarNumber &&
    currentPatient?.incomeDocumentUrl
  )
    return null;
  ...
  const validateStep = () => {
    ...
    if (step === 1) {
      ...
      if (!form.guardianMobile.trim() || form.guardianMobile.length !== 10) {
  ...
  const handleSubmit = async () => {
    ...
    setIsLoading(true);
    try {
      await submitKYC({ ...form, incomeDocument: docFile || new File([], 'demo-kyc.pdf') });
    } catch {
      // Error toast is shown in submitKYC; keep form state intact for user correction/retry.
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <Card className="w-full max-w-lg mx-auto border-gray-100 shadow-lg rounded-2xl">
      <CardHeader className="pb-2">
        <div className="flex items-center gap-2 mb-1">
          ...
          <CardTitle className="text-gray-800 font-normal text-lg">
            Patient Registration & KYC
          </CardTitle>
        </div>
          ...
          {STEPS.map((s, i) => {
            ...
            return (
              <div key={i} className="flex items-center flex-1">
                <div
                  className={`flex items-center gap-1 px-2 py-1 rounded-lg text-xs font-normal transition-all ${active ? 'bg-blue-600 text-white' : done ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-400'}`}
                >
                ...
                {i < 3 && (
                  <div
                    className={`h-px flex-1 mx-1 ${i < step ? 'bg-green-300' : 'bg-gray-200'}`}
                  />
                )}
              </div>
            );
      ...
      <CardContent className="pt-4 space-y-4">
          ...
          <div className="space-y-3">
            <div className="space-y-1">
              <Label className={labelCls}>
                ...
                <span className="text-gray-400 text-xs">(as per ID / Aadhaar)</span>
              </Label>
              <Input
                className={inputCls}
                placeholder="Exact name as on ID document"
                value={form.name}
            ...
            <div className="space-y-1">
              ...
              <Select value={form.govtIdType || ''} onValueChange={(v) => set('govtIdType', v)}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="Jan Dhan Account Passbook">
            ...
            <div className="space-y-1">
              <Label className={labelCls}>
                ...
                {form.govtIdType || 'Govt ID'} Number <span className="text-red-400">*</span>
              </Label>
              <Input
                className={inputCls}
                placeholder={
                  form.govtIdType === 'Aadhaar Card' ? 'XXXX XXXX XXXX' : 'Enter ID number'
                }
              ...
              <div className="bg-gray-50/50 rounded-xl p-3 border border-gray-100">
                ...
                <div className="space-y-3">
                  <div className="space-y-1">
                    ...
                    <Select
                      value={form.hospitalPartner || ''}
                      onValueChange={(v) => set('hospitalPartner', v)}
                    >
                  ...
                  <div className="space-y-1">
                    ...
                    <Input
                      className="h-8 text-xs border-gray-200 rounded-lg bg-white"
                      placeholder="e.g. Leukemia"
                      value={form.criticalIllness}
                      onChange={(e) => set('criticalIllness', e.target.value)}
                    />
                  </div>
                </div>
              </div>
        ...
        {step === 1 && (
          <div className="space-y-3">
            <div className="space-y-1">
              <Label className={labelCls}>
                Guardian Relation <span className="text-red-400">*</span>
              </Label>
              <Select
              ...
              <p className="text-xs text-gray-400">
                Father, mother, or any one guardian is sufficient
              </p>
            </div>
            <div className="space-y-1">
              <Label className={labelCls}>
                {form.guardianRelation}'s Full Name <span className="text-red-400">*</span>
              </Label>
              <Input
                className={inputCls}
                placeholder={`Enter ${form.guardianRelation?.toLowerCase()}'s name`}
            ...
            <div className="space-y-1">
              <Label className={labelCls}>
                {form.guardianRelation}'s Mobile Number <span className="text-red-400">*</span>
              </Label>
              <Input
                className={inputCls}
                type="tel"
                ...
                maxLength={10}
                onChange={(e) =>
                  set('guardianMobile', e.target.value.replace(/\D/g, '').slice(0, 10))
                }
              />
            ...
            <div className="space-y-1">
              ...
              <Select
                ...
                onValueChange={(v) => set('annualFamilyIncome', v)}
              >
                <SelectTrigger className={inputCls}>
            ...
            <div className="space-y-1">
              <Label className={labelCls}>
                ...
                Income Proof Document <span className="text-gray-400 text-xs">(optional)</span>
              </Label>
              <Input
                className={inputCls}
                type="file"
              ...
              {docFile && (
                <p className="text-xs text-green-600 flex items-center gap-1">
                  <FileText className="w-3 h-3" /> {docFile.name}
                </p>
              )}
            </div>
          </div>
        ...
        {step === 3 && (
          <div className="space-y-4">
            <div className="bg-blue-50/30 rounded-2xl p-4 text-gray-600 space-y-4 border border-blue-100/50">
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-y-3 gap-x-4 text-xs">
                  <div className="col-span-2 text-gray-400 font-medium uppercase tracking-wider text-[10px]">
                    Patient Information
                  </div>
                  <div className="text-gray-400">Name</div>
                  <div className="font-medium text-gray-800 text-right">{form.name || '---'}</div>
                  <div className="text-gray-400">DOB</div>
                  <div className="font-medium text-gray-800 text-right">
                    {form.dateOfBirth || '---'}
                  </div>
                  <div className="text-gray-400">Gender</div>
                  <div className="font-medium text-gray-800 text-right">{form.gender}</div>
                  <div className="text-gray-400">{form.govtIdType}</div>
                  <div className="font-medium text-gray-800 text-right truncate">
                    {form.aadhaarNumber || '---'}
                  </div>
                  ...
                  <div className="text-gray-400">{form.guardianRelation}</div>
                  <div className="font-medium text-gray-800 text-right">
                    {form.guardianName || '---'}
                  </div>
                  ...
                  <div className="text-gray-400">Annual Income</div>
                  <div className="font-medium text-gray-800 text-right">
                    {INCOME_LABELS[form.annualFamilyIncome as string] || '---'}
                  </div>
            ...
            <p className="text-[10px] text-gray-400 text-center px-4 leading-relaxed">
              By clicking "Submit for Verification", you agree that the information provided is
              correct and matches your official government documents.
            </p>
          </div>
        )}
          ...
          <div className="flex gap-2 pt-2">
            {step > 0 && (
              <Button
                variant="outline"
                onClick={back}
                className="flex-1 border-gray-200 rounded-xl font-normal"
              >
            ...
            {step < 3 ? (
              <Button
                onClick={next}
                className="flex-1 bg-blue-600 hover:bg-blue-700 rounded-xl font-normal"
              >
